...
let numSwitch = document.getElementById('num');
let sceneSwitch = document.getElementById('scene');
let mirrorSwitch = document.getElementById('mirrored');
let currentCamera = {
    targets: {
        doubles: {
            positionX: 0,
...
left.onclick = () => {
    let mm = 1;
    if (currentCamera.source.scaleX < 0)
        mm = -1;
    const check = mm > 0 ? currentCamera.source.cropLeft : currentCamera.source.cropRight;
    if (check > 0) {
...
right.onclick = () => {
    ...
    if (currentCamera.source.scaleX < 0)
        mm = -1;
    const check = mm > 0 ? currentCamera.source.cropRight : currentCamera.source.cropLeft;
    if (check > 0) {
        currentCamera.source.cropRight -= horizStep * mm;
        currentCamera.source.cropLeft += horizStep * mm;
        camChange();
...
function correctCam() {
    ...
    const horizTotalCrop = currentCamera.source.cropLeft + currentCamera.source.cropRight;
    const vertTotalCrop = currentCamera.source.cropTop + currentCamera.source.cropBottom;
    ...
    const targetAspectRatio = currentCamera.targets[type].width / currentCamera.targets[type].height;
...
function makeCropPossible(crop) {
    if (crop.cropLeft < 0)
        crop.cropLeft = 0;
    if (crop.cropRight < 0)
        crop.cropRight = 0;
    if (crop.cropTop < 0)
        crop.cropTop = 0;
    if (crop.cropBottom < 0)
        crop.cropBottom = 0;
    const horizTotalCrop = crop.cropLeft + crop.cropRight;
    const horizCropCorrection = (currentCamera.source.sourceWidth - 2) / horizTotalCrop;
    ...
    const vertCropCorrection = (currentCamera.source.sourceHeight - 2) / vertTotalCrop;
