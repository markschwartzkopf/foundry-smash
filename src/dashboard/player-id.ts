...
function drawSwitchPlayers(
	players: players,
	type: playType,
	switchPlayers: switchPlayer
) {
	let switchPlayerDiv = document.getElementById(
		'switch-players'
	)! as HTMLDivElement;
	let switchLabels = document.getElementById('switch-labels') as HTMLDivElement;
	if (switchLabels) switchPlayerDiv.removeChild(switchLabels);
	...
	let singlesSwap = document.getElementById(
		'single-switch-swap'
	)! as HTMLDivElement;
	let doublesSwap = document.getElementById(
		'double-switch-swap'
	)! as HTMLDivElement;
	if (type == 'singles') {
...
function drawInputArea(players: players, type: playType) {
	...
	for (let x = 0; x < inputNumber; x++) {
		...
		for (let y = 1; y <= inputNumber; y++) {
			...
			if (players[x].color == y) colorSelector.checked = true;
			colorSelector.onclick = () => {
				if (colorSelector.checked) {
					...
					if (inputNumber == 4) {
						...
						} else playersRep.value![x + 1].color = y;
					}
				} else playersRep.value![x].color = 0;
			};
		...
		if (x < inputNumber - 1) {
			...
			swap.onclick = () => {
				NodeCG.waitForReplicants(playersRep, playTypeRep, switchPlayerRep).then(
					() => {
						let newArray: players = JSON.parse(
							JSON.stringify(playersRep.value)
						);
  ...
  if (!newVal) return;
	for (let x = 0; x < newVal.length; x++) {
		for (let y = 0; y < smashColors.length; y++) {
			...
			let switchLabel = document.getElementById(
				'switch-player-' + x
			) as HTMLDivElement;
			if (switchLabel) {
...
inc1.onclick = () => {
  NodeCG.waitForReplicants(scoreRep).then(() => {
    if (scoreRep.value) scoreRep.value[0]++
  })
}

inc2.onclick = () => {
  NodeCG.waitForReplicants(scoreRep).then(() => {
    if (scoreRep.value) scoreRep.value[1]++
  })
}

dec1.onclick = () => {
  NodeCG.waitForReplicants(scoreRep).then(() => {
    if (scoreRep.value && scoreRep.value[0] > 0) scoreRep.value[0]--
  })
}

dec2.onclick = () => {
  NodeCG.waitForReplicants(scoreRep).then(() => {
    if (scoreRep.value && scoreRep.value[1] > 0) scoreRep.value[1]--
  })
}
...
document.getElementById('switch-1-2')!.onclick = () => {
	NodeCG.waitForReplicants(switchPlayerRep).then(() => {
		let newArray = JSON.parse(
			JSON.stringify(switchPlayerRep.value)
		) as switchPlayer;
