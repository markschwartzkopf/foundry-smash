...
}) as AbstractReplicant<'server', x32settings, NodeCG.Replicant.OptionsWithDefault<x32settings>, false>;
const commentaryChannels = ['03', '04'];
const commentaryBusses = ['09', '11'];

let connected = false;
let x32Socket = dgram.createSocket({ type: 'udp4', reuseAddr: true });
...
function send(msg: Buffer) {
  ...
  } else console.error("Can't send data to X32 unless connected");
}
