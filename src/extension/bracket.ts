...
import { challongeMatch, smashggApiMatch, smashggApiSeed, smashggMatch } from './types/server';
    ...
    fetch(ChallongeMethodUrl('matches'), { method: 'GET' })
      ...
      .then((resp) => {
        ...
        if (Array.isArray(resp)) {
          ...
          if (highRoundMatch) {
            if (finalFinalMatch && finalFinalMatch.winner_id) highRoundMatch.winner_id = finalFinalMatch?.winner_id;
...
function getSmasgGGPhases() {
  return new Promise<any>((res, rej) => {
    ...
    const matches: { [id: string]: smashggApiMatch } = {};
    const seeds: { [id: string]: smashggApiSeed } = {};
    const phaseInfo: {
      [phaseGroupId: string]: {
        lastWinnerRound: number;
        lastLoserRound: number;
        placements: { [placement: string]: smashggApiMatch[] };
        allMatches: smashggApiMatch[];
      };
    } = {};
    smashggFetch(smashPhaseQuery, tournamentRep.value!)
      .then((resp) => {
        if (
          ...
          (resp.event.phases as any[]).every((x) => typeof x.id == 'number' && typeof x.name == 'string')
        ) {
          let phaseArray = resp.event.phases as { id: number; name: string }[];
          Promise.all(
            phaseArray.map((x) => {
              return getSmashggPhaseMatches(x.id);
            }),
          )
            .then((phaseMatches: unknown[][]) => {
            ...
            .then((phaseSeeds: unknown[][]) => {
              phaseSeeds.forEach((phase) => {
                phase.forEach((seed) => {
                  if (isSmashggApiSeed(seed)) {
                    seeds[String(seed.id)] = seed;
                  } else rej('Bad start.gg seed');
                });
              });
              function seedCompare(a: { seed: number; round: number }, b: { seed: number; round: number }) {
                return b.round - a.round + 1 / (a.seed - b.seed);
              }
              function getHighestSeed(
                match: smashggApiMatch,
                losers?: boolean,
                koRound?: number | null,
              ): { seed: number; round: number } {
                if (!koRound) koRound = null;
                if (match.round > 0 && losers) koRound = match.round;
                const seedNums: { seed: number; round: number }[] = [];
              ...
              Object.entries(phaseInfo).forEach(([phaseGroupId, phase]) => {
                phase.allMatches.forEach((match) => {
                  if (
                    (match.round === phase.lastWinnerRound || match.round === phase.lastLoserRound) &&
                    match.wPlacement !== null
                  ) {
                    const placement = match.wPlacement.toString();
                    if (!phase.placements[placement]) phase.placements[placement] = [];
                ...
                Object.entries(phase.placements).forEach(([placement, matches]) => {
                  matches.sort((a, b) => {
                    ...
                    if (seedA.round) return seedCompare(seedB, seedA);
              ...
              Object.entries(matches).forEach(([id, apiMatch]) => {
                apiMatch.slots.forEach((slot) => {
                  if (slot.prereqType === 'seed') {
                    const seed = seeds[String(slot.prereqId)];
                    if (seed) {
                      if (!seed.progressionSource) return;
                      ...
                      if (phaseInfoItem) {
                        ...
                        if (placement) {
                          ...
                          if (match) {
                            ...
                            console.log(
                              `No match found for seed ${slot.prereqId}, looking for index ${index} in placement ${seed.progressionSource.originPlacement} in phase group ${phaseGroupId}`,
                            );
                        } else
                          console.log(
                            `No phase placement info for seed ${slot.prereqId}, looking for placement ${seed.progressionSource.originPlacement} in phase group ${phaseGroupId}`,
                          );
                      } else
                        console.log(`No phase info for seed ${slot.prereqId}, looking for phase group ${phaseGroupId}`);
                    } else console.log(`No seed match for match ${id}, looking for seed ${slot.prereqId}`);
                  }
                });
                let score1 = 0;
                let score2 = 0;
                if (
                  apiMatch.slots[0].standing &&
                ...
                if (match.player1_id && playerIds[match.player1_id]) match.player1_name = playerIds[match.player1_id];
                if (match.player2_id && playerIds[match.player2_id]) match.player2_name = playerIds[match.player2_id];
              ...
              if (highRoundMatch) {
                if (finalFinalMatch) highRoundMatch.winner_id = finalFinalMatch.winner_id;
...
function getSmashggPhaseSeeds(phaseId: number, existingArray?: any[], startpage?: number) {
  return new Promise<any[]>((res, rej) => {
    const page = startpage || 1;
    /* console.log(
      `Getting phase seeds for phase ${phaseId} page ${page}. Existing array length: ${
        existingArray ? existingArray.length : 0
      }`,
    ); */
    let smashPhaseSeedQuery =
      '{phase(id: ' +
      phaseId +
      '){seeds(query:{page:' +
      page +
      ...
      .then((resp: unknown) => {
        if (
          ...
          if (resp.phase.seeds.pageInfo.totalPages > page) {
            ...
            res(resp.phase.seeds.nodes as any[]);
          }
        } else {
...
function getSmashggPhaseMatches(phaseId: number, existingArray?: any[], startpage?: number) {
  return new Promise<any[]>((res, rej) => {
    const page = startpage || 1;
    /* console.log(
...
function isSmashggApiSeed(x: unknown): x is smashggApiSeed {
  if (
    !!x &&
    typeof x === 'object' &&
    x !== null &&
...
function isSmashggApiMatch(match: unknown): match is smashggApiMatch {
  if (
    !!match &&
    typeof match === 'object' &&
    (typeof (match as smashggApiMatch).id == 'number' || typeof (match as smashggApiMatch).id == 'string') &&
    typeof (match as smashggApiMatch).round == 'number' &&
    (typeof (match as smashggApiMatch).winnerId == 'number' || (match as smashggApiMatch).winnerId == null) &&
    (match as smashggApiMatch).slots
  ) {
    let rtn = true;
    let slots = (match as smashggApiMatch).slots as unknown;
