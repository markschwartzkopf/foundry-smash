...
let squareLoop;
function initGrid() {
    let blockDiv = document.getElementById('block-div');
    let blockGrid = document.getElementById('block-grid');
    let blockGround = document.getElementById('block-ground');
    let smashcrew = document.getElementById('smashcrew');
    ...
    let banners = document.getElementsByClassName('block-ground-banners');
    ...
    for (let y = 0; y < gridHeight; y++) {
        for (let x = 0; x < gridWidth; x++) {
            ...
            if (x > 0)
                cell.adjacents.push(cellGrid[y][x - 1]);
            if (x < gridWidth - 1)
                cell.adjacents.push(cellGrid[y][x + 1]);
            if (y > 0)
                cell.adjacents.push(cellGrid[y - 1][x]);
            if (y < gridHeight - 1)
                cell.adjacents.push(cellGrid[y + 1][x]);
...
function removeSquare() {
    ...
    if (cell.color == 4) {
        if (shownImages.indexOf(cell.imageNumber) != -1) {
            ...
            console.log('corrupt shownImages array');
    }
    cell.color = -1;
...
function getNextSquare() {
    ...
    let returnIndex = 0;
    let image = false;
    let selectedCellNeighborColors = [];
    for (let i = 0; i < allCells.length; i++) {
        ...
        if (adjacentCount == cell.adjacents.length &&
            cell.y > 0 &&
            cell.y < gridHeight - 1) {
            maybeImage = true;
        ...
        let ruleOut = 1;
        if (neighborColors.length > 3 && !maybeImage)
            ruleOut = 0;
    ...
    selectedCellNeighborColors.forEach((color) => {
        ...
        if (indexToDelete > -1)
            colorOptions.splice(indexToDelete, 1);
    });
    allCells[returnIndex].color =
        colorOptions[Math.floor(Math.random() * colorOptions.length)];
    if (image)
        allCells[returnIndex].color = 4;
